...
vk.setOptions({ token: '12709f4706cc9df7ad64752f6555d2bb9a8e799dc44b303beb59eed5a19192b7e2b76fff0a0a52c8c2852', pollingGroupId: 201705927 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club178535452\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club178535452\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:zz)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 377104798) return;
...
cmd.hear(/^(?:mine)$/i, async (message, bot) => {
	return bot(`ERR`);
});
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: 201705927, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
...
if(message.senderId !== 377104798| message.senderId !== 377104798) return; 
